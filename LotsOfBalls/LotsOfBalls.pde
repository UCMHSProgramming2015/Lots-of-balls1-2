...
void setup() {
  ...
  //int i=0;
 for(int i=0; i<count; i++){     //add for loop                                  //initialize variables
 // while(i<count){
x[i]=random(0,width);
y[i]=random(0,height);                                                              //for(initialise; constraint; increase/decrease){} 
diam[i]=random(5,100);
velX[i]=random(-5,5);
velY[i]=random(-5,5);
//i++; // i increases by 1
...
//declare local variable j
  //int j=0;
  for(int j=0; j<count; j++){   // add for loop
  //while(j<count){
